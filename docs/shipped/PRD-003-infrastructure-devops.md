...
Shipped 2026-04-22. All infrastructure provisioned in `ap-southeast-2` via Terraform. Both staging and prod environments live. CI/CD pipeline wired to GitHub (`mildmanneredme/nightingale`) with OIDC-based auth (no long-lived secrets).

**Acceptance criteria status:**

| Criterion | Status |
|---|---|
| All resources in ap-southeast-2 with IaC committed to repo | ✅ |
| Staging and prod environments operational | ✅ |
| CI/CD: lint → test → build → staging deploy on every PR | ✅ |
| Production deploy requires manual approval | ✅ (`mildmanneredme` set as required reviewer on `prod` GitHub environment) |
| No secrets in git history; all secrets in AWS Secrets Manager | ✅ |
| RDS reachable only from application subnet; no public endpoint | ✅ |
| All S3 buckets encrypted (SSE-KMS), public access blocked | ✅ |
| Audit log S3 bucket: object lock + 7-year COMPLIANCE retention | ✅ |
| CloudTrail and CloudWatch alarms active | ✅ |
| TLS 1.3 enforced on all external endpoints | ⚠️ Partial — ALB configured with `ELBSecurityPolicy-TLS13-1-2-2021-06`; HTTPS listener is inactive pending domain registration and ACM certificate provisioning |

**Known deviations from PRD:**
- RDS instance class is `db.t3.micro` (not `db.t3.medium` as specified) — free-tier account limitation. Upgrade to `db.t3.medium` when account is upgraded.
- `multi_az = false` in both staging and prod currently — upgrade prod to `multi_az = true` when account is upgraded.
- Backup retention is 1 day (not 30 days) — free-tier restriction. Increase to 30 days post-upgrade.

**Infrastructure created:**
- Terraform state: `s3://nightingale-tfstate-682812950646` + DynamoDB lock `nightingale-tf-lock`
- AWS Account: `682812950646` | Region: `ap-southeast-2`
- GitHub repo: `mildmanneredme/nightingale`
...
- [x] All resources provisioned in ap-southeast-2 with IaC (Terraform or AWS CDK) committed to repo
- [x] Staging and prod environments both operational
- [x] CI/CD pipeline runs on every PR: lint → test → build → staging deploy
- [x] Production deploy requires manual approval
- [x] No secrets in git history; all secrets in AWS Secrets Manager
- [x] RDS instance reachable only from application subnet; no public endpoint
- [x] All S3 buckets encrypted, public access blocked
- [x] Audit log S3 bucket has object lock + 7-year retention configured
- [x] CloudTrail and CloudWatch alarms active
- [~] TLS 1.3 enforced on all external endpoints — ALB policy set; pending domain + ACM certificate

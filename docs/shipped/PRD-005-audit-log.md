...
- [x] `audit_log` table defined in `infra/database/migrations/001_audit_log.sql` with append-only constraint (CHECK on actor_role, AHPRA non-null for doctors at DB layer)
- [x] `app_role` PostgreSQL role created with INSERT + SELECT only on `audit_log` — no UPDATE or DELETE granted
- [x] Doctor approval records require `ahpra_number`; DB-level CHECK enforces `actor_role != 'doctor' OR ahpra_number IS NOT NULL`
- [x] Indexes on `consultation_id`, `actor_id`, `event_type`, `created_at DESC` for query performance
- [x] S3 audit bucket provisioned with WORM object lock (COMPLIANCE mode, 7-year retention) — `nightingale-audit-{env}` (PRD-003)
- [x] Hourly EventBridge rule triggers audit-export Lambda (`rate(1 hour)`)
- [x] Audit-export Lambda reads `audit_log` records from last hour and writes NDJSON to S3 at `audit-export/YYYY/MM/DD/HH00.ndjson`
- [x] Lambda IAM role: Secrets Manager (DB credentials), S3 PutObject (audit-export/*), KMS (encrypt), EC2 VPC networking
